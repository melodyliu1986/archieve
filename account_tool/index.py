...
def index():
    ...
    if search_eng_form.validate_on_submit():
        logging.info("====== Search Subscription SKUs - Search by Eng Product Id(s) ======")
        ...
        # Handle EngProductId(s) order issue, search 69,180 have the same result with 180,69.
        search_str1 = ""
        for eng in eng_data.split(","):
            search_str11 = "(fn.Lower(SkuEntry.eng_product_ids) % '{0}' " \
                           "| fn.Lower(SkuEntry.eng_product_ids) % '{1}'" \
                           "| fn.Lower(SkuEntry.eng_product_ids) % '{2}')".\
                           format("%,{0},".format(eng), "%,{0}".format(eng), "{0},%".format(eng))
            print "&"*20
            print search_str11

            search_str111 = " & (fn.Lower(SkuEntry.eng_product_ids) % '%,{0}'".format(eng)
            search_str1 += search_str111 + search_str11

        search_str1 = ("&" + search_str1).replace("& &", "")

        print "#"*20
        print search_str1

        sku_matrix_new = []
        for row in SkuEntry.select().where(eval(search_str1)):
            meta = {}
            meta["SKU"] = row.id
            meta['Product Hierarchy: Product Category'] = row.ph_category
            meta['Product Hierarchy: Product Line'] = row.ph_product_line
            ...
            '''
            if len(eng_data.split(",")) == 1:
                sku_matrix.append(meta)
            else:
                sku_matrix_new.append(meta)
                if meta in sku_matrix_tmp and meta not in sku_matrix:
                    sku_matrix.append(meta)
                sku_matrix_tmp.append(meta)
            '''

        if len(sku_matrix) == 0:
                sku_failed_matrix = str_append(sku_failed_matrix, ("Eng Product Id(s) "+eng_data))
                logging.error("Failed to get sku %s" % sku_failed_matrix)

        logging.info("====== End: Search Subscription SKUs - Search by Eng Product Id(s) ======")

        if len(sku_matrix) == 0:
            return render_template('search_failed.html', data=search_attribute, sku_failed_matrix=sku_failed_matrix)
        else:
